...
public class ArgsTest {
  ...
  //Added test to check Input format
  @Test
  public void testImproperInputFormatBoolean() throws Exception {
	  try {
	      new Args("x", new String[]{"-x 1"});
	      fail();
	    } catch (ArgsException e) {
	      assertEquals(MALFORMED_INPUT_STRING, e.getErrorCode());
	      assertEquals('x', e.getErrorArgumentId());
	    }
  }
  
  @Test
  public void testImproperInputFormat() throws Exception {
	  try {
	      new Args("x,y*,z#", new String[]{"-x y 344 z sdfh"});
	      fail();
	    } catch (ArgsException e) {
	      assertEquals(MALFORMED_INPUT_STRING, e.getErrorCode());
